...
labels_train1=labels_train[0:1000]
labels_valid1=labels_valid[0:200]


images_train=[]
for count in range(0,1000):
    image_train=imread("./train/"+str(count)+".png")
    #print(morph.binary_dilation(image_train,iterations=1))
    images_train.append(misc.imresize(image_train,(img_size,img_size))/255.0)
images_valid=[]
for count in range(0,200):
    image_valid=imread("./valid/"+str(count)+".png")

    images_valid.append(misc.imresize(image_valid,(img_size,img_size))/255.0)
...
py_x = model(X, w, w2, w3, w4,w5, w_out, prob_conv, prob_hidden,training_phase)
...
with tf.Session() as sess:
    ...
    saver.restore(sess, "../model0.92.ckpt")
    ...
    for i in range(100):
        ...
        #print(np.argmax(testing_Y[test_indices],axis=1))
        
        prediction=sess.run(predict_op, feed_dict={X: testing_X[test_indices],
        ...
        accuracy=np.mean(np.argmax(testing_Y[test_indices], axis=1) == prediction)
